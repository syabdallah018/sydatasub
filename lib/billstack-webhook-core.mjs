...
export function normalizeBillstackWebhookPayload(input) {
  ...
  if (payload.data.account) {
    if (Array.isArray(payload.data.account) && payload.data.account[0]?.account_number) {
      accountNumber = String(payload.data.account[0].account_number).trim();
    } else if (typeof payload.data.account === "object" && payload.data.account.account_number) {
      accountNumber = String(payload.data.account.account_number).trim();
    }
  }
  if (!accountNumber && payload.data.account_number) {
