...
const DEBUG_LOGS_ENABLED = process.env.DEBUG_PROVIDER_LOGS === "1" || process.env.DEBUG_ALRAHUZ_LOGS === "1";
...
function redactHeaders(headers) {
  ...
  if (safe.Authorization) {
    safe.Authorization = "Token [redacted]";
  }
  return safe;
}

function logAlrahuzRequest(url, body, headers) {
...
function logAlrahuzResponse(url, response) {
  if (!DEBUG_LOGS_ENABLED) return;
...
async function postJson(path, body, options = {}) {
  ...
  logAlrahuzRequest(url, body, headers);
  ...
  logAlrahuzResponse(url, response);
